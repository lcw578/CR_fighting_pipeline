...
CANNON = 27000000
BATTLE_RAM = 26000036
ANGRY_BARBARIANS = 26000043


@dataclass(frozen=True)
class EvolutionBinding:
    card_id: int
    # None adopts the native variant data_id observed in card_runtime and then
    # matches deployed entities by unit name alone.
    form_card_id: int | None
    # None matches any native_data_global_id for the unit name.
    unit_id: int | None
    unit_name: str
    base_form: str
    evolved_form: str
    cost: int
    cycles: int = 2
    # Additional spawned (native_data_global_id, native_data_name) variants.
    extra_units: tuple[tuple[int, str], ...] = ()
...
BINDINGS = {
    ...
    BATTLE_RAM: EvolutionBinding(BATTLE_RAM, None, None,
        'BattleRam_EV1', 'BattleRam', 'BattleRam_EV1', 4, cycles=2),
    ANGRY_BARBARIANS: EvolutionBinding(ANGRY_BARBARIANS, 13000043, 1043953944,
        'AngryBarbarian_EV1', 'AngryBarbarians', 'AngryBarbarians_EV1', 6, cycles=1,
        extra_units=((2548884181, 'AngryBarbarian_EV1_2'),)),
}
...
class CardEvolutionTracker:
    ...
    def observe(self, selections, bundle, tick):
        ...
        variant_id_ok = binding.form_card_id is None or variants and variants[0].get('data_id') == binding.form_card_id
        if (row.get('variants_known') is not True or len(variants) != 1 or spec is None
            or not variant_id_ok or variants[0].get('cycle_required') != binding.cycles
...
def is_evolved_entity(entity, card_id):
    ...
    if not binding:
        return False
    if binding.form_card_id is not None and entity.get('card_id') != binding.form_card_id:
        return False
    unit_id = entity.get('native_data_global_id')
    unit_name = entity.get('native_data_name')
    if unit_name == binding.unit_name and (binding.unit_id is None or unit_id == binding.unit_id):
        return True
    return any(unit_id == uid and unit_name == name for uid, name in binding.extra_units)
