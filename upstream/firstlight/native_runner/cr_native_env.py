...
class NativeClashEnv:
    ...
    def live_hand(self) -> dict[str, Any]:
        """Return live hand-slot indices and elixir diagnostics.

        The probe maps each hand vector element to a deck-slot index.  This is
        intentionally a diagnostic envelope, not an ObservationV1 publisher:
        callers must map only the actor's own indices through its known deck
        before constructing a FAIR observation.
        """

        return self._request("live-hand")
