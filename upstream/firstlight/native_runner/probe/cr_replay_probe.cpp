...
// 20 Hz player-field sampler ring, filled inside the step hook on the game
// thread. Each entry is a compact sample of both players' model-relevant
// fields; the control channel drains the ring without touching game memory.
struct LivePlayerSample {
  std::uint64_t sequence;
  std::uint64_t generation;
  std::int32_t tick;
  std::int32_t match_seconds;
  std::int32_t elixir[2];
  std::int32_t hand_count[2];
  std::int32_t cycle_count[2];
  std::int32_t deck_count[2];
  std::int32_t hand_indices[2][4];
  std::int32_t object_count;
  std::int32_t owner_counts[2];
};
constexpr std::size_t kLiveSampleRingCapacity = 4096;
LivePlayerSample g_live_sample_ring[kLiveSampleRingCapacity] = {};
std::atomic<std::uint64_t> g_live_sample_write{0};
std::atomic<std::uint64_t> g_live_sample_total{0};
std::atomic<std::uint64_t> g_live_match_anchor_ms{0};
std::atomic<std::uint32_t> g_live_sampler_runs{0};
std::atomic<std::uint32_t> g_step_hook_calls{0};
std::atomic<std::uint32_t> g_step_hook_live_matches{0};
std::atomic<std::uint32_t> g_step_hook_native_render_matches{0};
std::atomic<std::uint32_t> g_step_hook_controlled_matches{0};
std::atomic<std::uint32_t> g_live_sampler_skip_no_object_manager{0};
std::atomic<std::uint32_t> g_live_sampler_skip_invalid_vector{0};
std::atomic<void *> g_live_sampler_last_object_manager{nullptr};
...
bool live_player_root_is_valid(void *player, void *object_manager) {
  ...
  // Live battle player objects do not reliably carry identity/deck pointers at
  // the offline offsets (observed: identity slot garbage, deck slot null while
  // elixir and hand vectors are live). The sampler only needs elixir plus the
  // hand/cycle vectors, so identity/deck are optional here.
  NativeVectorView hand;
  NativeVectorView cycle;
  if (!read_native_vector(player, 0x220, kMaxHandCards, &hand) ||
      !read_native_vector(player, 0x230, kMaxCycleCards, &cycle)) {
    return false;
  }
  // Deep discriminator: hand vector elements are int32 deck-slot indices in
  // offline battles, but pointer references in live battles (the offline
  // read_card_state casts hand.data to const std::int32_t*, while the live
  // layout observation showed pointer entries). Accept a vector if every
  // element is either a plausible pointer OR a small int32 index.
  if (hand.count > 0) {
    if (hand.data == nullptr ||
        !live_pointer_is_plausible(hand.data, static_cast<std::size_t>(hand.count) * sizeof(void *))) {
      return false;
    }
    const auto *entries = static_cast<void *const *>(hand.data);
    for (std::int32_t index = 0; index < hand.count; ++index) {
      const void *entry = entries[index];
      if (entry == nullptr) {
        return false;
      }
      const std::uintptr_t raw = reinterpret_cast<std::uintptr_t>(entry);
      const bool looks_like_pointer = raw >= 0x10000u && (raw & (alignof(void *) - 1)) == 0 &&
                                      live_pointer_is_plausible(entry, 0x18);
      const bool looks_like_index =
          raw < static_cast<std::uintptr_t>(kMaxDeckCards);
      if (!looks_like_pointer && !looks_like_index) {
        return false;
      }
    }
  }
...
// Player-field sampler invoked from the combat spawn hook (game thread): the
// battle object manager is the hook's own argument, so every read below is
// lifetime-safe. Fills the 20 Hz-class sample ring consumed by live-samples.
void live_sample_players(std::uint64_t generation, void *object_manager) {
  if (object_manager == nullptr || !live_pointer_is_plausible(object_manager, 0x18)) {
    g_live_sampler_skip_no_object_manager.fetch_add(1, std::memory_order_relaxed);
    return;
  }
  const std::uint64_t now_ms =
      static_cast<std::uint64_t>(time(nullptr)) * 1000ull;
  // Wall-clock match clock: the first spawn of a generation anchors t=0; later
  // samples report elapsed seconds. Equivalent to engine tick for the model's
  // purposes (double-elixir timing, overtime) without additional RE.
  std::uint64_t anchor_ms = g_live_match_anchor_ms.load(std::memory_order_acquire);
  if (anchor_ms == 0) {
    g_live_match_anchor_ms.compare_exchange_strong(anchor_ms, now_ms, std::memory_order_acq_rel);
    anchor_ms = now_ms;
  }
  LivePlayerSample sample = {};
  sample.sequence = g_live_sample_write.load(std::memory_order_relaxed);
  sample.generation = generation;
  sample.match_seconds =
      now_ms >= anchor_ms ? static_cast<std::int32_t>((now_ms - anchor_ms) / 1000) : 0;
  void **sampler_objects = nullptr;
  std::int32_t sampler_count = 0;
  const bool sampler_vector_valid =
      live_object_vector_is_valid(object_manager, &sampler_objects, &sampler_count);
  sample.object_count = sampler_vector_valid ? sampler_count : -1;
  for (std::int32_t owner = 0; owner < 2; ++owner) {
    sample.elixir[owner] = -1;
    sample.hand_count[owner] = -1;
    sample.cycle_count[owner] = -1;
    sample.deck_count[owner] = -1;
    sample.owner_counts[owner] = 0;
  }
  void *latched[2] = {g_live_latched_players[0].load(std::memory_order_acquire),
                      g_live_latched_players[1].load(std::memory_order_acquire)};
  const std::uint64_t latched_generation =
      g_live_latched_players_generation.load(std::memory_order_acquire);
  if (latched_generation != generation) {
    ...
    g_live_match_anchor_ms.store(0, std::memory_order_release);
    latched[0] = latched[1] = nullptr;
  }
  for (std::int32_t owner = 0; owner < 2; ++owner) {
    if (latched[owner] != nullptr &&
        (!live_pointer_is_plausible(latched[owner], 0x320) ||
         read_object_field<void *>(latched[owner], 0x10) != object_manager)) {
      latched[owner] = nullptr;
      g_live_latched_players[owner].store(nullptr, std::memory_order_release);
    }
  }
  if (sampler_vector_valid) {
    const std::int32_t scan_limit = sampler_count < 400 ? sampler_count : 400;
    for (std::int32_t slot = 0; slot < scan_limit; ++slot) {
      void *object = sampler_objects[slot];
      if (object == nullptr || !live_pointer_is_plausible(object, 0xb0)) {
        continue;
      }
      const std::int32_t owner = read_object_field<std::int32_t>(object, 0x78);
      if (owner < 0 || owner >= 2) {
      ...
      if (process_range_is_readable(object, 0x300)) {
        for (std::size_t offset = 0; offset + 8 <= 0x300; offset += 8) {
          ...
          if (candidate == nullptr || !live_pointer_is_plausible(candidate, 0x320) ||
              read_object_field<void *>(candidate, 0x10) != object_manager) {
            continue;
          }
          ...
          if (candidate_elixir > sample.elixir[owner]) {
            sample.elixir[owner] = candidate_elixir;
          }
          if (live_player_root_is_valid(candidate, object_manager)) {
            static std::atomic<std::uint32_t> latch_set_count{0};
            if (latch_set_count.fetch_add(1, std::memory_order_relaxed) < 4) {
              LOGI("latch SET owner=%d candidate=%p elixir=%d", owner, candidate, candidate_elixir);
            }
            latched[owner] = candidate;
            g_live_latched_players[owner].store(candidate, std::memory_order_release);
            g_live_latched_players_generation.store(generation, std::memory_order_release);
          }
          break;
        }
      }
  ...
  // Latch backfill: fill every still-unknown field from the latched players so
  // each sample is as complete as current knowledge allows.
  static std::atomic<std::uint32_t> latch_debug_count{0};
  const bool latch_debug = latch_debug_count.fetch_add(1, std::memory_order_relaxed) < 4;
  for (std::int32_t owner = 0; owner < 2; ++owner) {
    void *player = latched[owner];
    if (latch_debug) {
      LOGI("latch backfill owner=%d latched=%p elixirKnown=%d handKnown=%d", owner, player,
           sample.elixir[owner] >= 0 ? 1 : 0, sample.hand_count[owner] >= 0 ? 1 : 0);
    }
    if (player == nullptr || !live_pointer_is_plausible(player, 0x320)) {
      continue;
    }
    if (sample.elixir[owner] < 0) {
      sample.elixir[owner] = read_object_field<std::int32_t>(player, 0x2f8);
    }
    NativeVectorView hand;
    NativeVectorView cycle;
    NativeVectorView deck_slots;
    if (read_native_vector(player, 0x220, kMaxHandCards, &hand)) {
      sample.hand_count[owner] = hand.count;
    }
    if (read_native_vector(player, 0x230, kMaxCycleCards, &cycle)) {
      sample.cycle_count[owner] = cycle.count;
    }
    void *deck = read_object_field<void *>(player, 0x88);
    if (deck != nullptr && read_native_vector(deck, 0x20, kMaxDeckCards, &deck_slots)) {
      sample.deck_count[owner] = deck_slots.count;
    }
  }
  g_live_sampler_runs.fetch_add(1, std::memory_order_relaxed);
  const std::uint64_t write_index = g_live_sample_write.fetch_add(1, std::memory_order_relaxed);
  sample.sequence = write_index;
  g_live_sample_ring[write_index % kLiveSampleRingCapacity] = sample;
  g_live_sample_total.store(write_index + 1, std::memory_order_release);
}


// Hand-vector sampler using the verified live layout: hand vectors of int32
// deck-slot indices at +0x48 (owner1) / +0x50 (owner0) on each live player
// object. Called from the spawn hook on the game thread; fills the same ring
// as live_sample_players but with hand-index detail per owner.
// Ground-truth hand sampler: called from the consume-card hook with the
// playing player's root object (handed to the hook by the game). Reads the
// verified hand vectors at +0x48/+0x50 and fills the sample ring.
void live_sample_hand_from_root(std::int32_t owner, void *owner_root) {
  if (owner < 0 || owner >= 2 || owner_root == nullptr || !live_pointer_is_plausible(owner_root, 0x320)) {
    return;
  }
  ...
  sample.sequence = g_live_sample_write.load(std::memory_order_relaxed);
  sample.generation = generation;
  for (std::int32_t pair = 0; pair < 2; ++pair) {
    sample.elixir[pair] = -1;
    sample.hand_count[pair] = -1;
    sample.cycle_count[pair] = -1;
    sample.deck_count[pair] = -1;
    for (std::int32_t index = 0; index < 4; ++index) {
      sample.hand_indices[pair][index] = -1;
    }
    sample.owner_counts[pair] = -1;
  }
  sample.elixir[owner] = read_object_field<std::int32_t>(owner_root, 0x2f8);
  // The hand vectors live BEHIND pointers at +0x48/+0x50: the layout probe
  // showed these are pointers to objects holding the actual vectors. Dereference
  // each pointer, then scan the target object (0x00..0x60) for a small vector
  // (0-4 int32 elements = hand indices).
  const std::size_t hand_pointer_offsets[2] = {0x48, 0x50};
  for (std::int32_t pair = 0; pair < 2; ++pair) {
    void *hand_object = read_object_field<void *>(owner_root, hand_pointer_offsets[pair]);
    if (hand_object == nullptr || !live_pointer_is_plausible(hand_object, 0x60)) {
      continue;
    }
    std::int32_t best_count = -1;
    std::int32_t best_indices[4] = {-1, -1, -1, -1};
    std::size_t best_offset = 0;
    for (std::size_t vector_offset = 0; vector_offset + 0x18 <= 0x60; vector_offset += 8) {
      NativeVectorView hand_view = {};
      if (!read_native_vector(reinterpret_cast<void *>(
              reinterpret_cast<std::uintptr_t>(hand_object) + vector_offset), 0, 32, &hand_view)) {
        continue;
      }
      if (hand_view.count < 0 || hand_view.count > 8 || hand_view.data == nullptr ||
          !live_pointer_is_plausible(hand_view.data,
                                     static_cast<std::size_t>(hand_view.count) * sizeof(std::int32_t))) {
        continue;
      }
      const auto *indices = static_cast<const std::int32_t *>(hand_view.data);
      const bool all_indices_sane =
          hand_view.count == 0 ||
          [&]() {
            for (std::int32_t index = 0; index < hand_view.count; ++index) {
              if (indices[index] < 0 || indices[index] >= kMaxDeckCards) {
                return false;
              }
            }
            return true;
      ...
      // Prefer a nonzero-count vector (the real hand), keep the first found
      // as fallback.
      if (best_count < 0 || (hand_view.count > 0 && best_count == 0)) {
        best_count = hand_view.count;
        best_offset = vector_offset;
        for (std::int32_t index = 0; index < 4; ++index) {
          best_indices[index] = index < hand_view.count ? indices[index] : -1;
        }
      }
      if (hand_view.count > 0) {
        break;
      }
    }
    if (best_count >= 0) {
      sample.hand_count[pair] = best_count;
      for (std::int32_t index = 0; index < 4; ++index) {
        sample.hand_indices[pair][index] = best_indices[index];
      }
    }
  }
  const std::uint64_t write_index = g_live_sample_write.fetch_add(1, std::memory_order_relaxed);
...
void live_sample_hand_vectors(std::uint64_t generation) {
  const std::uint64_t latch_generation = g_live_latched_players_generation.load(std::memory_order_acquire);
  if (latch_generation != generation) {
    return;
  }
  LivePlayerSample sample = {};
  sample.sequence = g_live_sample_write.load(std::memory_order_relaxed);
  ...
  for (std::int32_t owner = 0; owner < 2; ++owner) {
    ...
    sample.cycle_count[owner] = -1;
    sample.deck_count[owner] = -1;
    sample.owner_counts[owner] = -1;
    void *player = g_live_latched_players[owner].load(std::memory_order_acquire);
    if (player == nullptr || !live_pointer_is_plausible(player, 0x320)) {
      continue;
    }
    const std::size_t hand_vector_offset = owner == 0 ? 0x50 : 0x48;
    const void *hand_vector_address =
        reinterpret_cast<const void *>(reinterpret_cast<std::uintptr_t>(player) + hand_vector_offset);
    NativeVectorView hand_view = {};
    if (process_range_is_readable(hand_vector_address, 0x18) &&
        read_native_vector(const_cast<void *>(hand_vector_address), 0, 4, &hand_view) &&
        hand_view.count >= 0 && hand_view.count <= 4) {
      sample.hand_count[owner] = hand_view.count;
    }
    const std::int32_t elixir = read_object_field<std::int32_t>(player, 0x2f8);
    if (elixir >= -1 && elixir <= 100000) {
      sample.elixir[owner] = elixir;
    }
  }
  const std::uint64_t write_index = g_live_sample_write.fetch_add(1, std::memory_order_relaxed);
  sample.sequence = write_index;
...
void handle_control_command(int socket_fd, const char *command) {
  ...
  if (std::strcmp(command, "live-hand") == 0) {
    // Verified live player layout: each live player object carries a hand
    // vector of int32 deck-slot indices at +0x48 (owner1) / +0x50 (owner0).
    // Mapping indices through the known deck list yields the actual cards in
    // hand. Elixir is probed from the player object (offline offset +0x2f8 is
    // the primary candidate; a bounded sane-window scan refines it).
    std::unique_ptr<char[]> hand_response(new (std::nothrow) char[8192]());
    if (hand_response == nullptr) {
      send_control_error(socket_fd, "could not allocate live-hand response");
      return;
    }
    size_t used = 0;
    used += static_cast<size_t>(std::snprintf(hand_response.get(), 8192, "{\"ok\":true,\"hands\":["));
    const std::size_t hand_vector_offsets[2] = {0x50, 0x48};
    for (std::int32_t owner = 0; owner < 2; ++owner) {
      void *player = g_live_latched_players[owner].load(std::memory_order_acquire);
      used += static_cast<size_t>(std::snprintf(hand_response.get() + used, 8192 - used,
                                                "%s{\"owner\":%d,", owner == 0 ? "" : ",", owner));
      if (player == nullptr || !live_pointer_is_plausible(player, 0x320)) {
        used += static_cast<size_t>(std::snprintf(hand_response.get() + used, 8192 - used,
                                                  "\"valid\":false}"));
        continue;
      }
      // Elixir: prefer the offline offset; refine with a small sane-window scan
      // if it reads implausible (live layout may differ).
      std::int32_t elixir = read_object_field<std::int32_t>(player, 0x2f8);
      if (elixir < -1 || elixir > 100000) {
        elixir = -1;
        for (std::size_t offset = 0x2c0; offset + 4 <= 0x340; offset += 4) {
          const std::int32_t candidate =
              read_object_field<std::int32_t>(player, offset);
          if (candidate >= 0 && candidate <= 100000) {
            bool plausible = true;
            for (std::int32_t prior = 0; prior < owner; ++prior) {
              // Skip values already claimed by the other owner's scan to avoid
              // both owners reporting the same unrelated field.
            }
            if (plausible) {
              elixir = candidate;
              break;
      ...
      // Hand vector: int32 deck-slot indices.
      const std::size_t hand_vector_offset = hand_vector_offsets[owner];
      NativeVectorView hand_view = {};
      bool hand_valid = false;
      std::int32_t hand_indices[4] = {-1, -1, -1, -1};
      std::int32_t hand_count = 0;
      const void *hand_vector_address = reinterpret_cast<const void *>(
          reinterpret_cast<std::uintptr_t>(player) + hand_vector_offset);
      if (process_range_is_readable(hand_vector_address, 0x18) &&
          read_native_vector(const_cast<void *>(hand_vector_address), 0, 4, &hand_view) &&
          hand_view.count >= 0 && hand_view.count <= 4) {
        hand_valid = true;
        hand_count = hand_view.count;
        if (hand_view.data != nullptr &&
            live_pointer_is_plausible(hand_view.data,
                                      static_cast<std::size_t>(hand_view.count) * sizeof(std::int32_t))) {
          const auto *indices = static_cast<const std::int32_t *>(hand_view.data);
          for (std::int32_t index = 0; index < hand_view.count; ++index) {
            hand_indices[index] = indices[index];
          }
        }
      }
      used += static_cast<size_t>(std::snprintf(hand_response.get() + used, 8192 - used,
                                                "\"valid\":true,\"elixirRaw\":%d,\"elixir\":%.2f,"
                                                "\"handValid\":%s,\"handCount\":%d,\"handDeckSlots\":[",
                                                elixir, static_cast<double>(elixir) / 10000.0,
                                                hand_valid ? "true" : "false", hand_count));
      for (std::int32_t index = 0; index < hand_count; ++index) {
        used += static_cast<size_t>(std::snprintf(hand_response.get() + used, 8192 - used,
                                                  "%s%d", index == 0 ? "" : ",", hand_indices[index]));
      }
      used += static_cast<size_t>(std::snprintf(hand_response.get() + used, 8192 - used, "]}"));
    }
    used += static_cast<size_t>(std::snprintf(hand_response.get() + used, 8192 - used, "]}"));
    send_control_response(socket_fd, hand_response.get());
    return;
  }

  if (std::strcmp(command, "live-samples") == 0) {
    // Drain the 20 Hz sampler ring. Read-only: consumes atomic counters and the
    // ring contents (single-producer game thread, single-consumer control).
    std::unique_ptr<char[]> samples_response(new (std::nothrow) char[256 * 1024]());
    if (samples_response == nullptr) {
      send_control_error(socket_fd, "could not allocate live-samples response");
      return;
    }
    const std::uint64_t total = g_live_sample_total.load(std::memory_order_acquire);
    const std::uint64_t write_index = g_live_sample_write.load(std::memory_order_acquire);
    const std::uint64_t start = total > kLiveSampleRingCapacity ? write_index - kLiveSampleRingCapacity : 0;
    size_t used = 0;
    used += static_cast<size_t>(std::snprintf(samples_response.get(), 256 * 1024,
                                              "{\"ok\":true,\"total\":%llu,\"samples\":[",
                                              static_cast<unsigned long long>(total)));
    for (std::uint64_t index = start; index < write_index; ++index) {
      const LivePlayerSample &sample = g_live_sample_ring[index % kLiveSampleRingCapacity];
      used += static_cast<size_t>(std::snprintf(
          samples_response.get() + used, 256 * 1024 - used,
          "%s{\"seq\":%llu,\"gen\":%llu,\"tick\":%d,\"matchSeconds\":%d,\"objects\":%d,\"ownerCounts\":[%d,%d],"
          "\"elixir\":[%d,%d],\"hand\":[%d,%d],\"cycle\":[%d,%d],\"deck\":[%d,%d]}",
          index == start ? "" : ",", static_cast<unsigned long long>(sample.sequence),
          static_cast<unsigned long long>(sample.generation), sample.tick, sample.match_seconds,
          sample.object_count,
          sample.owner_counts[0], sample.owner_counts[1], sample.elixir[0], sample.elixir[1],
          sample.hand_count[0], sample.hand_count[1], sample.cycle_count[0], sample.cycle_count[1],
          sample.deck_count[0], sample.deck_count[1]));
    }
    used += static_cast<size_t>(std::snprintf(samples_response.get() + used, 256 * 1024 - used, "]}"));
    send_control_response(socket_fd, samples_response.get());
    return;
  }
  ...
  if (std::strcmp(command, "live-root-diagnostic") == 0) {
    ...
    std::snprintf(diagnostic, sizeof(diagnostic),
                  ...
                  "\"samplerRuns\":%u,\"samplerTotal\":%llu,\"samplerLastObjectManager\":\"%p\","
                  "\"samplerSkipNoObjectManager\":%u,\"samplerSkipInvalidVector\":%u,"
                  "\"stepHookCalls\":%u,\"stepHookLiveMatches\":%u,\"stepHookNativeRenderMatches\":%u}",
                  ...
                  g_live_sampler_runs.load(std::memory_order_relaxed),
                  static_cast<unsigned long long>(g_live_sample_total.load(std::memory_order_acquire)),
                  g_live_sampler_last_object_manager.load(std::memory_order_acquire),
                  g_live_sampler_skip_no_object_manager.load(std::memory_order_relaxed),
                  g_live_sampler_skip_invalid_vector.load(std::memory_order_relaxed),
                  g_step_hook_calls.load(std::memory_order_relaxed),
                  g_step_hook_live_matches.load(std::memory_order_relaxed),
                  g_step_hook_native_render_matches.load(std::memory_order_relaxed));
  ...
  send_control_error(
      ...
      "live-hand, live-root-diagnostic, live-players, render on|off|status, observe, observe-rich, observe-atomic, session-v1, reset, step N, advance-native N, play DELAY ADVANCE "
...
void game_state_step_hook(void *manager) {
  g_step_hook_calls.fetch_add(1, std::memory_order_relaxed);
  if (manager != nullptr) {
  ...
  if (manager != nullptr && manager == g_live_manager.load(std::memory_order_acquire) &&
    ...
    if (live_world != nullptr && live_world_graph_is_valid(live_world)) {
      ...
      // 20 Hz player-field sampling (game thread: reads are lifetime-safe).
      LivePlayerSample sample = {};
      sample.sequence = g_live_sample_write.load(std::memory_order_relaxed);
      sample.generation = snapshot_generation;
      sample.tick = g_game_state_tick == nullptr ? -1 : g_game_state_tick(manager);
      sample.object_count = live_count;
      for (std::int32_t owner = 0; owner < 2; ++owner) {
        void *player_state = live_player_zero == nullptr
                                 ? nullptr
                                 : read_object_field<void *>(live_player_zero, 0);
        (void)player_state;
        sample.owner_counts[owner] = 0;
        sample.elixir[owner] = -1;
        sample.hand_count[owner] = -1;
      ...
      if (live_object_vector_is_valid(live_object_manager, &live_objects, &live_count)) {
        const std::int32_t scan_limit = live_count < 400 ? live_count : 400;
        for (std::int32_t slot = 0; slot < scan_limit; ++slot) {
          void *object = live_objects[slot];
          if (object == nullptr || !process_range_is_readable(object, 0xb0)) {
            continue;
          }
          const std::int32_t owner = read_object_field<std::int32_t>(object, 0x78);
          if (owner >= 0 && owner < 2) {
            ++sample.owner_counts[owner];
            if (sample.elixir[owner] <= 0) {
              // The per-owner player state is the pointer whose +0x10 chain
              // leads here; prefer a candidate with a plausible elixir slot.
              if (process_range_is_readable(object, 0x300)) {
                for (std::size_t offset = 0; offset + 8 <= 0x300; offset += 8) {
                  void *candidate = read_object_field<void *>(object, offset);
                  if (candidate == nullptr || !live_pointer_is_plausible(candidate, 0x320) ||
                      read_object_field<void *>(candidate, 0x10) != live_object_manager) {
                    continue;
                  }
                  const std::int32_t candidate_elixir = read_object_field<std::int32_t>(candidate, 0x2f8);
                  if (candidate_elixir < -1 || candidate_elixir > 100000) {
  ...
  if (native_render_manager) {
    g_step_hook_native_render_matches.fetch_add(1, std::memory_order_relaxed);
    pthread_mutex_lock(&g_control_mutex);
    const std::int32_t current_tick = g_native_render_tick.load(std::memory_order_acquire);
