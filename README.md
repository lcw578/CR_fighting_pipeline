...
**本仓库自包含部署端**：FirstLight 推理运行时与冻结卡牌数据已内置于 `upstream/firstlight/`，
全部模型权重位于 `weights/`——克隆后无需再单独获取上游代码或权重，Windows 上开箱即用。
训练端不随本仓库分发（来源与哈希溯源见 `upstream.lock.json`）。
...
FirstLight 推理运行时、冻结目录数据与模型权重已随本仓库分发（`upstream/firstlight/`、`weights/`），无需单独准备。
...
2. **填设置**：复制 `settings.example.json` 为 `settings.local.json`，填入你的 ADB 路径、实例端口；
   按提示在手动对局中运行 `tools\inspect_players.py` 确认账号，用 `tools\coordinate_view.py` 核对坐标。
3. **装探针并开战**：`powershell -ExecutionPolicy Bypass -File .\probe\deploy_probe.ps1` 安装稳定探针（会重启游戏），
   然后双击 `start_agent.bat` 选择运行方式；多局自动化用：

   ```powershell
   # 无限连打（天梯结算页用"再来一场"直接衔接下一局）
   .venv\Scripts\python.exe tools\multi_match.py --matches 100000 --checkpoint active_il --rematch
...
1. 运行 `setup.ps1` 安装本项目 Python 环境。
